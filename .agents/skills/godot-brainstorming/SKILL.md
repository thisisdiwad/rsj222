...
1. **Inject AGENTS.md** — Add the GodotPrompter integration section to the project's AGENTS.md (see AGENTS.md Injection section below). This ensures all subagents and future sessions know to use GodotPrompter skills. Skip if the `## GodotPrompter` section already exists.
...
When preparing for implementation (Step 4), add the following section to the project's AGENTS.md. Check for an existing `## GodotPrompter` heading first — if present, skip.
