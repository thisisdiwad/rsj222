...
npx Codex-flow benchmark-run --suite comprehensive --duration 300

# Execute specific benchmark
npx Codex-flow benchmark-run --suite throughput --iterations 10

# Compare with baseline
npx Codex-flow benchmark-compare --current <results> --baseline <baseline>

# Quality assessment
npx Codex-flow quality-assess --target swarm-performance --criteria throughput,latency

# Performance validation
npx Codex-flow validate-performance --results <file> --criteria <file>
```

### Regression Detection Commands
```bash
# Detect performance regressions
npx Codex-flow detect-regression --current <results> --historical <data>

# Set up automated regression monitoring
npx Codex-flow regression-monitor --enable --sensitivity 0.95

# Analyze error patterns
npx Codex-flow error-analysis --logs <log-files>
```
