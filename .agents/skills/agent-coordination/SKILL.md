...
### Spawn Agent
```bash
npx Codex-flow agent spawn --type coder --name my-coder
```

### List Agents
```bash
npx Codex-flow agent list --filter active
```

### Agent Status
```bash
npx Codex-flow agent status --id agent-123
```

### Agent Metrics
```bash
npx Codex-flow agent metrics --id agent-123
```

### Stop Agent
```bash
npx Codex-flow agent stop --id agent-123
```

### Pool Management
```bash
npx Codex-flow agent pool --size 5 --type coder
```
