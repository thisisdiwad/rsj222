...
npx Codex-flow performance-report --format detailed --timeframe 24h

# Real-time bottleneck analysis
npx Codex-flow bottleneck-analyze --component swarm-coordination

# Health check all components
npx Codex-flow health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
npx Codex-flow metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
npx Codex-flow sla-monitor --service swarm-coordination --threshold 99.9
```

### Alert Configuration
```bash
# Configure performance alerts
npx Codex-flow alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
npx Codex-flow anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
npx Codex-flow notification-config --channels ["slack", "email", "webhook"]
```
