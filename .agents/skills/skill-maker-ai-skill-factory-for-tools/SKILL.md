...
4. **Registering with Codex** - Immediate availability as new skill
...
class PatternRecognizer:
    ...
    def analyze_tool_semantics(self, tool_spec: ToolSpec) -> dict:
        """Use Codex to understand tool semantics and find SPI opportunities."""
...
Deployer:
    def register_skill(self, skill_md: str, tool_name: str) -> bool:
        """Register generated skill with Codex."""
        ...
        result = subprocess.run(
            ["Codex", "code", "--register-skill", tool_name.lower()],
            capture_output=True,
            text=True
        )
...
async def make_skill_for_tool(tool_name: str, github_url: str = None) -> bool:
    ...
    if success:
        ...
        print(f"   Usage: Codex --skill {tool_name.lower()}")
    else:
