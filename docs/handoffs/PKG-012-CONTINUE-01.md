...
PKG-009 i zawiera własne `.git`/`.gitignore`; pozostaw cały backup nietknięty,
nie używaj jego plików Git i nie wykonuj cleanupu. Główny `.git` usunięto
23.09.2026, głównego `.gitignore` nie było. **Trwały zakaz Git:** zapisuj
bezpośrednio na dysku, nie wykonuj żadnych poleceń Git, nie twórz repozytorium
ani konfiguracji; zakaz może
zmienić tylko nowe, wyraźne polecenie użytkownika. Dawne odtworzenie `.git`
(commit `9b4a5ad`, 25 niedopasowanych blobów) to historia, nie pozwolenie.
...
twierdzenia o brakach nie są bieżącym stanem. Poprzednia wersja PKG-012
pozostaje w `docs/handoffs/PKG-012-rev01.md`.
